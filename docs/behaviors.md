...
- **Typing is verified.** Keystrokes go in as real HID presses; a key that does not land is retyped
  against the device's read-back (`data-dd-device-value`) — on iOS and on Android's masked fields. So
  wait for that echo before submitting.
