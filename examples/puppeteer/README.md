# Puppeteer against a real simulator
...
from labels. Puppeteer drives it like any webpage; no Appium, no
...
- The mirror refreshes from the native tree (~300ms while active), so
  `waitForSelector` works — new screens appear on the next refresh
